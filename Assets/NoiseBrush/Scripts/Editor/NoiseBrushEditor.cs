	...
	public override void OnInspectorGUI () 
	{
		...
		if (script.guiShowGenerator)
		{
			...
			else
			{
				...
				if (preset.noise_seed != tempSeed) { Noise.seed = tempSeed; preset.noise_seed = tempSeed; UnityEngine.Random.InitState(tempSeed); }
		...
		if (script.guiShowTextures)
		{
			...
			TerrainLayer[] terrainLayers = script.terrain.terrainData.terrainLayers;
			Texture2D[] textures = new Texture2D[terrainLayers != null ? terrainLayers.Length : 0];
			for (int i=0; i<textures.Length; i++) 
			{
				if (terrainLayers[i] != null)
					textures[i] = terrainLayers[i].diffuseTexture;
			}
			#else
